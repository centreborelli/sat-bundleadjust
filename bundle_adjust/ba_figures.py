...
def display_skysat_acquisition_scheme(r_h=5, overlap_ratio=0.2, save_pgf=False):
    ...
    # left strip
    rectangle = np.array([[0.0, 0.0], [0.0, r_h], [r_w, r_h], [r_w, 0.0]])
    for n in range(9):
    ...
    for n in range(4):
        plt.plot(
            [r_coords[n, 0], r_coords[n, 0] + r_w / 1.2],
            [r_coords[n, 1], r_coords[n, 1] + r_h / 0.7],
            "--k",
            alpha=0.3,
        )
    rect = patches.Rectangle(
        (r_coords[0, 0] + r_w / 1.2, r_coords[0, 1] + r_h / 0.7),
        r_w,
        r_h,
        linewidth=1,
        edgecolor="k",
    ...
    ax.add_patch(rect)
    ax.text(
        r_coords[0, 0] + r_w / 1.2 + r_w / 22,
        r_coords[0, 1] + r_h / 0.7 + r_h / 6,
        "single frame\n     image",
        fontsize=FS,
    )
    ...
    if save_pgf:
        plt.savefig(
            "skysat_acquisition.png", pad_inches=0, bbox_inches="tight", dpi=200
        )
...
def display_skysat_sensor_scheme(r_h=5, save_pgf=False):
    ...
    def draw_sensor(x, y, sensor_id):
        ...
        ax.text(
            x + r_w / 2.5,
            y + subband_h * 1 - subband_h + subband_h / 6,
            "NIR",
            fontsize=FS,
        )
        ax.text(
            x + r_w / 2.5,
            y + subband_h * 2 - subband_h + subband_h / 6,
            "red",
            fontsize=FS,
        )
        ax.text(
            x + r_w / 2.8,
            y + subband_h * 3 - subband_h + subband_h / 6,
            "green",
            fontsize=FS,
        )
        ax.text(
            x + r_w / 2.6,
            y + subband_h * 4 - subband_h + subband_h / 6,
            "blue",
            fontsize=FS,
        )
        ax.text(
            x + r_w / 2.6,
            y + subband_h * 6.5 - subband_h + subband_h / 6,
            "PAN",
            fontsize=FS,
        )
        ax.text(
            x + r_w / 3.4,
            y + subband_h * 9 - subband_h + subband_h / 2,
            "sensor " + sensor_id,
            fontsize=FS,
        )
    ...
    plt.arrow(
        x, y, dx=0, dy=-r_h / 2 - r_h / 6, head_width=0.4, head_length=0.4, fc="k"
    )
    ax.text(
        r_w - 0.5 + r_w / 4,
        -r_h / 2 - r_h / 20,
...
def display_row_of_dsms(
    dsms, vmin=None, vmax=None, c="cividis", aois=None, custom_cb=True
):
...
def plot_RBCT_evolution_over_time(
    dsm_stock, dsm_labels, start_date="2020-01-14", end_date="2020-07-01"
):
    ...
    date_dict = dict(
        [
            (l, d)
            for l, d in zip(all_labels, all_dates)
    ...
    all_dates = [datetime.datetime.strptime(l, "%Y-%m-%d") for l in all_labels]
    gt_stock = [
        gt_stock[i] for i in range(len(gt_stock)) if gt_labels[i] in date_dict.keys()
    ]
    gt_labels = [l for l in gt_labels if l in date_dict.keys()]
    gt_labels_pos = [list(date_dict.keys()).index(l) for l in gt_labels]
    dsm_stock = [
        dsm_stock[i] for i in range(len(dsm_stock)) if dsm_labels[i] in date_dict.keys()
    ]
...
def compute_matches_over_time(scene, timeline_indices, out_dir):
    ...
    def run_only_feature_tracking(scene, verbose=False):
        ...
        offsets = [
            ...
            for c in scene.ba_data["crops"]
        ]
        cameras, _ = loader.approx_perspective_projection_matrices(
            scene.ba_data["rpcs"], offsets, verbose=verbose
        )
        ...
        local_data = {
            "n_adj": scene.ba_data["n_adj"],
            "n_new": scene.ba_data["n_new"],
            "fnames": scene.ba_data["image_fnames"],
            "images": input_seq,
            "rpcs": scene.ba_data["rpcs"],
            "offsets": offsets,
            "footprints": footprints,
            "optical_centers": optical_centers,
            "masks": scene.ba_data["masks"],
        }

        with suppress_stdout():
            ft_pipeline = FeatureTracksPipeline(
                scene.ba_data["input_dir"],
                scene.ba_data["output_dir"],
                local_data,
                config=scene.tracks_config,
                satellite=True,
            )
    ...
    for i in timeline_indices:
        ...
        n_matches = (
            len(feature_tracks["pairwise_matches"])
            if feature_tracks["C"] is not None
            else 0
        )
        diff_days = 0.0
        ...
        for j in np.array(timeline_indices)[np.array(timeline_indices) > i]:
            ...
            scene.set_ba_input_data(
                current_timeline_indices, ba_dir, ba_dir, 0, verbose
            )
        ...
        print(
            "{}/{} done in {:.2f} seconds".format(
                counter, len(timeline_indices), running_time
            )
        )

    np.savetxt(
        os.path.join(out_dir, "matches_over_time.txt"), np.array(matches_over_time)
    )
    np.savetxt(
        os.path.join(out_dir, "matches_over_time_timeline_indices.txt"),
        np.array(timeline_indices),
    )
...
def plot_matches_over_time(in_dir, scene, hist_consecutive_time_diff=True):
    ...
    timeline_indices = (
        np.loadtxt(os.path.join(in_dir, "matches_over_time_timeline_indices.txt"))
        .astype(int)
        .tolist()
    )
    ...
    y_avg_merged = np.repeat(
        [y_avg[i] + y_avg[i + 1] for i in np.arange(0, len(y_avg) - 1, 2)], 2
    )
...
def display_overlap_between_image_footprints(footprints, resolution):
    ...
    from bundle_adjust import data_loader, geojson_utils
    ...
    for f in footprints:
        ...
        utm_geojsons.append(
            {"type": "Polygon", "coordinates": [(coordinates).tolist()]}
        )
    ...
    for im_idx, x in enumerate(utm_geojsons):
        ...
        mask_footprints[:, :, im_idx] = data_loader.mask_from_shapely_polygons(
            [shapely_poly], (height, width)
        )
    ...
    cb = plt.colorbar(
        im, ax=ax, fraction=cb_frac, pad=cb_pad, aspect=cb_asp, ticks=cb_tick_pos
    )
...
def plot_teaser_rpc_fitting():
    ...
    from bundle_adjust import ba_rotate, camera_utils
    from bundle_adjust import data_loader as loader
    ...
    rotated_tmp = ba_rotate.euler_angles_to_R(
        0.17 / 2, 0.17 / 2, -0.17 / 3
    ) @ np.vstack([cols, lins, alts])
    rotated_tmp = rotated_tmp.T
    z_offset = 500
    ax.scatter(
        rotated_tmp[:, 0], rotated_tmp[:, 1], rotated_tmp[:, 2] + z_offset, marker="o"
    )
    ...
    ax.add_collection3d(
        Poly3DCollection(verts, facecolors="w", edgecolor="k", linewidths=1, alpha=0.0)
    )

    ax.scatter(
        image_points[:, 0], image_points[:, 1], [0] * len(image_points), marker="x"
    )
...
def plot_timeline(
    scene, timeline_indices=None, filename=None, date_label_freq=2, fs=22
):
    plt.rcParams.update(plt.rcParamsDefault)
    ...
    ax1.set_ylim(
        bottom=np.floor(min(diff_in_days)) - 0.2, top=np.ceil(max(diff_in_days)) + 0.7
    )
    ...
    plt.xticks(
        np.arange(n_dates)[::date_label_freq], np.array(dates)[::date_label_freq]
    )
    legend_labels = [
        "distance to previous date in day units",
        "number of images per date",
    ]
