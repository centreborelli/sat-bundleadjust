...
def warp_stereo_dsms(complete_dsm_fname, stereo_dsms_fnames):
    ...
    for dsm_idx, src_fname in enumerate(stereo_dsms_fnames):
        ...
        if os.path.isfile(dst_fname):
            continue
        os.system(
            "rio clip {} {} --like {} --with-complement --overwrite".format(*args)
        )
...
def compute_stat_for_specific_date_from_tiles(
    complete_dsm_fname,
    stereo_dsms_fnames,
    tile_size=500,
    output_dir=None,
    ...
    tiles_dir = os.path.join(
        output_dir, "tmp_tiles_{}_{}".format(loader.get_id(complete_dsm_fname), stat)
    )
        ...
        for col in x_lims:
            ...
            for fn in warp_fnames:
                with rasterio.open(fn) as src:
                    crops.append(
                        src.read(window=((row, limit_row), (col, limit_col))).squeeze()
                    )
    ...
    output_fn = complete_dsm_fname.replace(
        os.path.dirname(complete_dsm_fname), output_dir
    )
