...
def keypoints_to_utm_coords(features, rpcs, footprints, offsets):
    ...
    utm = []
    for features_i, rpc_i, footprint_i, offset_i in zip(
        features, rpcs, footprints, offsets
    ):
...
def compute_pairs_to_match(
    init_pairs, footprints, optical_centers, no_filter=False, verbose=True
):
    def set_pair(i, j):
    ...
    if verbose:
        print(
            "     {} / {} pairs suitable to match".format(
                len(pairs_to_match), len(init_pairs)
            )
        )
        print(
            "     {} / {} pairs suitable to triangulate".format(
                len(pairs_to_triangulate), len(init_pairs)
            )
        )
    return pairs_to_match, pairs_to_triangulate


def match_kp_within_utm_polygon(
    features_i,
    features_j,
    utm_i,
    utm_j,
    ...
    indices_i_poly_bool, all_indices_i = (
        np.logical_and(east_ok_i, north_ok_i),
        np.arange(utm_i.shape[0]),
    )
    ...
    indices_j_poly_bool, all_indices_j = (
        np.logical_and(east_ok_j, north_ok_j),
        np.arange(utm_j.shape[0]),
    )
    ...
    features_i_poly, features_j_poly = (
        features_i[indices_i_poly_bool],
        features_j[indices_j_poly_bool],
    )
    ...
    if method == "epipolar_based":
        matches_ij_poly, n = ft_s2p.s2p_match_SIFT(
            features_i_poly, features_j_poly, F, dst_thr=rel_thr, ransac_thr=ransac
        )
...
def filter_matches_inconsistent_utm_coords(matches_ij, utm_i, utm_j):
    ...
    utm_thr, success = get_elbow_value(
        all_utm_distances, max_outliers_percent=20, verbose=False
    )
...
def locally_match_SIFT_utm_coords(
    features_i, features_j, utm_i, utm_j, radius=30, sift_thr=250, ransac_thr=0.3
):
...
def match_stereo_pairs(
    ...
    for idx, pair in enumerate(pairs_to_match):
        ...
        if method == "epipolar_based":
            ...
            print(
                "{:4} matches ({}: {:4}, {}: {:4}) in pair {}{}".format(*args),
                flush=True,
            )
        elif method == "local_window":
            args = [
                n_matches,
                "imscript local window",
                n[0],
                "ransac",
                n[1],
                "utm",
            ...
            print(
                "{:4} matches ({}: {:4}, {}: {:4}, {}: {:4}) in pair {}{}".format(
                    *args
                ),
                flush=True,
            )
        ...
        if n_matches > 0:
            im_indices = np.vstack(
                (np.array([i] * n_matches), np.array([j] * n_matches))
            ).T
    ...
    pairwise_matches = np.hstack(
        (np.array(pairwise_matches_kp_indices), np.array(pairwise_matches_im_indices))
    )
    return pairwise_matches


def match_stereo_pairs_multiprocessing(
    pairs_to_match,
    features,
    footprints,
    utm_coords,
