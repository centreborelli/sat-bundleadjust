...
def ba_offsets_ransac_init_rpc_offset(cam_idx, pts3d, C, myrpcs, mycrops, ransac_thr=3.0, ransac_p=0.95, debug=False):
    ...
    if debug:
        ...
        for pt_idx in range(n_obs):
            ...
            init_reprj_err.append(np.linalg.norm(pts2d_img_obs[pt_idx,:]-pts2d_rpc_prj[pt_idx,:]))
            new_reprj_err.append(np.linalg.norm(pts2d_img_obs[pt_idx,:]-pts2d_new_prj))
        ...
        print('- Median RPC reprojection error     : {}'.format(np.median(init_reprj_err)))
        print('- Median RPC+offset reprojection error : {}\n'.format(np.median(new_reprj_err)))  
...
def ba_offsets_set_ba_params(C, pts3d, myrpcs, mycrops):
    ...
    for cam_idx in range(n_cam):
        ...
        offsets[cam_idx,:] = ba_offsets_ransac_init_rpc_offset(cam_idx, pts3d, C, myrpcs, mycrops, \
                                                               ransac_thr=3.0, ransac_p=0.95, debug=False)
    print('\nDone!\n')
...
def ba_offsets_fun(params, cam_ind, pts_ind, pts_2d, n_cam, n_pts, myrpcs):
    ...
    for c_idx in np.unique(cam_ind).tolist():
        ...
        col_prj[where_c_idx], row_prj[where_c_idx] = myrpcs[c_idx].projection(lon[where_c_idx], \
                                                                              lat[where_c_idx], \
                                                                              alt[where_c_idx]) 
    col_prj, row_prj = col_prj + cam_params_opt[cam_ind,0], row_prj + cam_params_opt[cam_ind,1]
