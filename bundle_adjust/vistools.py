...
def overlaymap(
    ...
    layer = ImageOverlay(
        url="%s" % (imagesurls[0]),
        bounds=[list(np.max(aoiY, axis=0)[::-1]), list(np.min(aoiY, axis=0)[::-1])],
    )
...
def display_cloud(xyz):
    ...
    os.system(
        "/home/rogermm14/PotreeConverter_PLY_toolchain/PotreeConverter/LAStools/bin/txt2las -parse xyz tmp/tmp.txt"
    )
    os.system(
        "/home/rogermm14/PotreeConverter_PLY_toolchain/PotreeConverter/build/PotreeConverter/PotreeConverter tmp/tmp.las -o point_clouds -p test --material ELEVATION --edl-enabled"
    )
