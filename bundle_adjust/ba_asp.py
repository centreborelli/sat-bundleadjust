...
def read_P_from_ASP_tsai_file(fname):   
    ...
    with open(fname) as f:
        ...
        K[0,0], K[1,1], K[0,1], K[0,2], K[1,2], K[2,2] = fx, fy, skew, cx, cy, pitch
...
def read_ASP_points_file(fname):  
    ...
    x, y, z = ba_utils.latlon_to_ecef_custom(np.array(lat), np.array(lon), np.array(alt))
...
def read_AMES_match_binary_file(fname, verbose=True):
    with open(fname, 'r') as f:
        ...
        for i in range(size1+size2):
            ...
            ip = {'x': x, 'y':y, 'ix':ix, 'iy':iy, 'orientation':orientation, 'scale':scale, 
                  'interest':interest, 'polarity':polarity, 'octave':octave, 'scale_lvl':scale_lvl, 
                  'size':descriptor_size, 'descriptor':descriptor}
...
def custom_get_rpc_rotation_center(rpc, col=0, row=0):
    ...
    long_scale_up = 10000; # This is a distance in meters approx from the top of the llh valid cube
...
def ames_project_adjusted_points(rpc_init, m_rotation, m_translation, pts_3d, m_rotation_center=None, m_rotation_inv=None):
...
def ames_fit_corrected_rpc(image_fname, adjust_fname, pts_3d_ecef, rpc=None, fit_proj_matrix=True, verbose=True):
    ...
    lat, lon, alt = ba_utils.ecef_to_latlon_custom(world_points[:, 0], world_points[:, 1], world_points[:, 2])
    world_points_lonlat = np.vstack([lon, lat, alt]).T
    
    m_translation, m_rotation, m_center = read_ASP_adjust_file(adjust_fname)
    image_points = ames_project_adjusted_points(rpc, m_rotation, m_translation, world_points, m_rotation_center=m_center)
    ...
    rmse_err = rpc_fit.calculate_RMSE_row_col(rpc_calib, world_points_lonlat, image_points)
...
def plot_reproj_pair(fname1, fname2, features1, features2, reproj_pts1, reproj_pts2):
    ...
    print('average reprojection error: {:.3f} / {:.3f}'.format( np.mean(err_1), np.mean(err_2) ))
    ...
    # before bundle adjustment
        ax1.plot([features1[k,0], reproj_pts1[k,0] ], [features1[k,1], reproj_pts1[k,1] ], 'r-', lw=3)
        ax1.plot(*features1[k], 'yx')
        # after bundle adjustment
        ax2.plot([features2[k,0], reproj_pts2[k,0] ], [features2[k,1], reproj_pts2[k,1]], 'r-', lw=3)
        ax2.plot(*features2[k], 'yx')
    plt.show()
    
def plot_reproj_pair_detail(fname1, fname2, features1, features2, reproj_pts1, reproj_pts2):
    ...
    err_1 = np.linalg.norm(reproj_pts1 - features1, axis=1)
    err_2 = np.linalg.norm(reproj_pts2 - features2, axis=1)
    avg_residuals = np.mean(np.hstack([abs(reproj_pts1 - features1), abs(reproj_pts2 - features2)]), axis=1)/2.0
    
    err_1[err_1 < 1e-3] = 0
    err_2[err_2 < 1e-3] = 0
    
    print('average reprojection error: {:.3f} / {:.3f}'.format( np.mean(err_1), np.mean(err_2) ))
    print('{} matches with reprojection error above 1'.format( np.sum(1* (((err_1 + err_2)/2.0) > 1.0))))
...
def read_point_log_csv(fname):
    ...
    x, y, z = ba_utils.latlon_to_ecef_custom(np.array(lat), np.array(lon), np.array(height_above_datum))
    pts_3d_ecef = np.vstack([x, y, z]).T
...
def read_cnet_csv(fname):
    ...
    with open(fname) as csvfile:
        ...
        for row in spamreader:
            ...
            obs_2d.append({'proj': np.array([[x,y] for x,y in zip(col, lin)]), 'fnames': geotif_fnames})
...
def read_residuals_raw_pixels(fname):
    ...
    with open(fname) as f:
        ...
        while line_counter < len(lines):
            ...
            residuals = np.array([np.array(get_doubles_from_line(lines[line_counter+i+1])) for i in range(n_points)])
