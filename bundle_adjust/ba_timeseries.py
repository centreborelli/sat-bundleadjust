...
from bundle_adjust import ba_core, ba_metrics, ba_utils
...
def run_s2p(thread_idx, input_config_files, aoi_lonlat_path=None):
    ...
    for dst_config_fn in input_config_files:
        ...
        print(
            "...reconstructing stereo DSMs (thread {} -> {}/{})".format(*to_print),
            flush=True,
        )
        dsms_computed += 1
        ...
        if aoi_lonlat_path is not None:
            ...
            mask = loader.get_binary_mask_from_aoi_lonlat_within_utm_bbx(
                dsm_utm_bbx, dsm_res, aoi_lonlat
            )
...
class Scene:
    def __init__(self, scene_config):
        ...
        if self.s2p_configs_dir != "" and not os.path.isdir(self.s2p_configs_dir):
            raise Error(
                's2p_config_dir "{}" does not exist'.format(self.s2p_configs_dir)
            )
        ...
        if self.s2p_configs_dir == "":
            self.timeline, self.aoi_lonlat = loader.load_scene_from_geotiff_dir(
                self.geotiff_dir,
                self.dst_dir,
            ...
            self.timeline, self.aoi_lonlat = loader.load_scene_from_s2p_configs(
                self.geotiff_dir,
        ...
        self.mask = loader.get_binary_mask_from_aoi_lonlat_within_utm_bbx(
            self.utm_bbx, 1.0, self.aoi_lonlat
        )
        ...
        print(
            "Number of acquisition dates: {} (from {} to {})".format(
                n_dates, start_date, end_date
            )
        )
        print(
            "Number of images: {}".format(
                np.sum([d["n_images"] for d in self.timeline])
            )
        )
    ...
    def display_aoi(self, zoom=14):
        geotools.display_lonlat_geojson_list_over_map(
            [self.aoi_lonlat], zoom_factor=zoom
        )
    ...
    def display_crops(self):
        ...
        if len(mycrops) > 0:
            vistools.display_gallery(
                [loader.custom_equalization(f["crop"]) for f in mycrops]
            )
        else:
    ...
    def display_image_masks(self):
        if not self.compute_aoi_masks:
            ...
            if len(mycrops) > 0:
                ...
                print(
                    "No crops have been loaded. Use load_data_from_date() to load them."
                )
    ...
    def check_adjusted_dates(self, input_dir, verbose=True):
        ...
        if os.path.exists(input_dir + "/filenames.pickle") and os.path.isdir(
            dir_adj_rpc
        ):
            ...
            if verbose:
                print(
                    "Found {} previously adjusted images in {}\n".format(
                        len(adj_fnames), self.dst_dir
                    )
                )

            datetimes_adj = [
                loader.get_acquisition_date(img_geotiff_path)
                for img_geotiff_path in adj_fnames
            ]
            ...
            if verbose:
                print(
                    "No previously adjusted data was found in {}\n".format(self.dst_dir)
                )
            prev_adj_data_found = False

        return prev_adj_data_found

    def load_data_from_dates(
        self, timeline_indices, input_dir, adjusted=False, verbose=True
    ):
        ...
        if verbose:
            print(
                n_cam,
                "{} images for bundle adjustment !".format(
                    "adjusted" if adjusted else "new"
                ),
            )
        ...
        if n_cam > 0:
            # get rpcs
            rpc_dir = (
                os.path.join(input_dir, "RPC_adj")
                if adjusted
                else os.path.join(self.dst_dir, "RPC_init")
            )
            rpc_suffix = "RPC_adj" if adjusted else "RPC"
            im_rpcs = loader.load_rpcs_from_dir(
                im_fnames, rpc_dir, suffix=rpc_suffix, verbose=verbose
            )
    ...
    def load_prev_adjusted_dates(
        self, t_idx, input_dir, previous_dates=1, verbose=True
    ):
        ...
        if found_adj_dates:
            ...
            all_prev_adj_t_indices = [
                idx for idx, d in enumerate(self.timeline) if d["adjusted"]
            ]
            closest_adj_t_indices = sorted(
                all_prev_adj_t_indices, key=lambda x: abs(x - t_idx)
            )
            adj_t_indices_to_use = closest_adj_t_indices[:previous_dates]
            adj_dates_to_use = ", ".join(
                [dt2str(self.timeline[k]["datetime"]) for k in adj_t_indices_to_use]
            )
            if verbose:
                print(
                    "Using {} previously adjusted date(s): {}\n".format(
                        len(adj_t_indices_to_use), adj_dates_to_use
                    )
                )
            self.load_data_from_dates(
                adj_t_indices_to_use, input_dir, adjusted=True, verbose=verbose
            )
    ...
    def set_ba_input_data(
        self, t_indices, input_dir, output_dir, previous_dates, verbose
    ):
        ...
        if self.compute_aoi_masks:
            self.ba_data["masks"] = [f["mask"] for f in self.mycrops_adj] + [
    ...
    def bundle_adjust(self, feature_detection=True, verbose=True, extra_outputs=False):
        ...
        if extra_outputs:
            image_weights = (
                self.ba_pipeline.compute_image_weights_after_bundle_adjustment()
            )
            return (
                elapsed_time,
                elapsed_time_FT,
                ...
                init_e,
                image_weights,
                self.ba_pipeline.pairs_to_triangulate,
            )
        else:
    ...
    def rm_tmp_files_after_ba(self, ba_method):
        ...
        os.system(
            "rm {}/{}/{}".format(self.dst_dir, ba_method, "pairs_triangulation.npy")
        )
    ...
    def print_ba_headline(
        self, timeline_indices, ba_method, previous_dates=0, next_dates=0
    ):
        print(
            "{} dates of the timeline were selected for bundle adjustment:".format(
                len(timeline_indices)
            )
        )
    ...
    def run_sequential_bundle_adjustment(
        ...
        self.print_ba_headline(
            timeline_indices, ba_method, previous_dates=previous_dates
        )
        ...
        for idx, t_idx in enumerate(timeline_indices):
            ...
            running_time, time_FT, n_tracks, _, _ = self.bundle_adjust(
                verbose=verbose, extra_outputs=False
            )
            pts_out_fn = "{}/pts3d_adj/{}_pts3d_adj.ply".format(
                ba_dir, self.timeline[t_idx]["id"]
            )
            ...
            print(
                "({}/{}) {} adjusted in {:.2f} seconds, {} ({:.3f}, {:.3f})".format(
                    *args
                ),
                flush=True,
            )
        ...
        print(
            "Average BA iterations per date: {}".format(
                int(np.ceil(np.mean(ba_iters_per_date)))
            )
        )
        print(
            "\nTOTAL TIME: {}\n".format(
                loader.get_time_in_hours_mins_secs(sum(time_per_date))
            ),
            flush=True,
        )
    ...
    def run_global_bundle_adjustment(
        ...
        self.tracks_config[
            "FT_predefined_pairs"
        ] = loader.load_pairs_from_same_date_and_next_dates(*args)
        ...
        print(
            "\nTOTAL TIME: {}\n".format(
                loader.get_time_in_hours_mins_secs(running_time)
            ),
            flush=True,
        )
    ...
    def extract_ba_data_indices_according_to_date(self, t_idx):
        ...
        ba_data_indices_t_id = [
            self.ba_data["image_fnames"].index(fn)
            for fn in self.timeline[t_idx]["fnames"]
        ]
        return ba_data_indices_t_id

    def compare_timeline_instances(
        self, timeline_indices_g1, timeline_indices_g2, ba_pipeline
    ):
        ...
        for node_idx, timeline_indices_g in enumerate(
            [timeline_indices_g1, timeline_indices_g2]
        ):
            g_dates = sorted(
                [self.timeline[t_idx]["datetime"] for t_idx in timeline_indices_g]
            )
            g_n_img = sorted(
                [self.timeline[t_idx]["n_images"] for t_idx in timeline_indices_g]
            )
            label = ""
            n_views = []
            for dt, n_img in zip(g_dates, g_n_img):
                ...
                label += "{}-{}-{} {}h\n".format(
                    dt.day, dt.month, str(dt.year)[-2:], dt.hour
                )
            label += "{} {}".format(sum(n_views), n_views)
            node_labels[node_idx] = label
            tmp = [
                self.extract_ba_data_indices_according_to_date(t_idx)
                for t_idx in timeline_indices_g
            ]
        ...
        for edge_idx, node_indices in enumerate(pairs_to_draw):
            ...
            (
                n_matches,
                n_matches_aoi,
            ) = ba_pipeline.get_number_of_matches_between_groups_of_views(
                g1_img_indices, g2_img_indices
            )

            current_edge = (i, j)
            if n_matches_aoi is not None:
                edge_labels[current_edge] = "{} matches\n({} aoi)".format(
                    n_matches, n_matches_aoi
                )
            else:
    ...
    def timeline_instances_diagram(self, timeline_indices, ba_pipeline, neighbors=1):
        ...
        nodes_dates = sorted(
            [self.timeline[t_idx]["datetime"] for t_idx in timeline_indices]
        )
        nodes_n_img = sorted(
            [self.timeline[t_idx]["n_images"] for t_idx in timeline_indices]
        )
        node_labels = {}
        for node_idx, dt in enumerate(nodes_dates):
            node_labels[node_idx] = "{}-{}\n{}\n{}h {}v".format(
                dt.day, dt.month, dt.year, dt.hour, nodes_n_img[node_idx]
            )
        nodes_img_indices_in_ba_data = [
            self.extract_ba_data_indices_according_to_date(t_idx)
            for t_idx in timeline_indices
        ...
        for edge_idx, node_indices in enumerate(pairs_to_draw):
            ...
            if edge_idx >= n_mandatory_edges:
                ...
                days, hours, _ = self.get_datetime_diff_in_days_hours(
                    nodes_dates[i], nodes_dates[j]
                )

                current_edge = (i, j)
                edges_of_interest.append(current_edge)
                edge_labels[current_edge] = "{}m\n{}d {}h".format(
                    n_matches, int(days), int(hours)
                )
    ...
    def set_rec4D_dir(self, ba_method, geotiff_label=None):
        ...
        rec4D_dir = "{}/{}/4D".format(
            self.dst_dir, ba_method if ba_method is not None else "init"
        )
    ...
    def reconstruct_dates(
        ...
        configs = []
        for t_idx in timeline_indices:
            src_configs = loader.load_s2p_configs_from_image_filenames(
                self.timeline[t_idx]["fnames"],
                self.s2p_configs_dir,
        ...
        for counter, t_idx in enumerate(timeline_indices):
            ...
            #    continue
            t_idx_crashes = self.reconstruct_date(
                t_idx,
                ba_method=ba_method,
                std=std,
                geotiff_label=geotiff_label,
            ...
            running_time_current_date = loader.get_time_in_hours_mins_secs(
                timeit.default_timer() - t0
            )
        ...
        total_running_time = loader.get_time_in_hours_mins_secs(
    ...
    def reconstruct_date(
        ...
        set_dst_s2p_dir = lambda t: t.replace(
            self.s2p_configs_dir, "s2p/{}".format(t_id)
        )
        dst_configs = [
            "{}/{}".format(rec4D_dir, set_dst_s2p_dir(fn)) for fn in src_configs
        ]
        n_dsms = len(src_configs)

        # prepare new s2p configs
        for dsm_idx, (src_config_fn, dst_config_fn) in enumerate(
            zip(src_configs, dst_configs)
        ):
            self.update_config_json_after_bundle_adjustment(
                src_config_fn, dst_config_fn, ba_method, verbose=verbose
            )

        n = int(np.ceil(n_dsms / n_s2p))
        s2p_aoi = self.dst_dir + "/AOI_init.pickle" if ba_method is None else None
        args_s2p = [
            (k, dst_configs[i : i + n], s2p_aoi)
            for k, i in enumerate(np.arange(0, len(dst_configs), n))
        ]
        ...
        if ba_method is None:
            ...
            aoi_lonlat_ba = loader.load_pickle(
                os.path.join(self.dst_dir, "{}/AOI_adj.pickle".format(ba_method))
            )
        utm_bbx_ba = geotools.utm_bbox_from_aoi_lonlat(aoi_lonlat_ba)
        dsm_resolution = float(
            loader.load_dict_from_json(src_configs[0])["dsm_resolution"]
        )

        # merge stereo dsms into output muti-view dsm
        dsm_path = os.path.join(rec4D_dir, "dsms/{}.tif".format(t_id))
        ply_list = glob.glob(
            "{}/s2p/{}/**/cloud.ply".format(rec4D_dir, t_id), recursive=True
        )
        ba_utils.run_plyflatten(
            ply_list,
            dsm_resolution,
            utm_bbx_ba,
            dsm_path,
            aoi_lonlat=aoi_lonlat_ba,
            std=std,
        )
    ...
    def load_reconstructed_DSMs(
        self, timeline_indices, ba_method, pc3dr=False, geotiff_label=None
    ):
        ...
        dsm_timeseries = []
        for t_idx in timeline_indices:
            dsm_fname = os.path.join(
                dsms_dir, "{}.tif".format(self.timeline[t_idx]["id"])
            )
            dsm_timeseries.append(np.array(Image.open(dsm_fname)))
        return np.dstack(dsm_timeseries)

    def compute_stats_over_time(
        self,
        timeline_indices,
        ba_method,
        pc3dr=False,
        ...
        output_dir = os.path.join(
            rec4D_dir, "pc3dr/metrics_over_time" if pc3dr else "metrics_over_time"
        )
        os.makedirs(output_dir, exist_ok=True)

        # get timeseries
        if use_cdsms:
            dsms_dir = os.path.join(
                rec4D_dir,
                "pc3dr/metrics/avg_per_date" if pc3dr else "metrics/avg_per_date",
            )
            dsm_timeseries = []
            for t_idx in timeline_indices:
                dsm_fname = os.path.join(
            ...
            dsm_timeseries_ndarray = self.load_reconstructed_DSMs(
                timeline_indices, ba_method, pc3dr=pc3dr, geotiff_label=geotiff_label
            )
    ...
    def compute_stat_per_date(
        ...
        pc3dr=False,
        geotiff_label=None,
        clean_tmp_warps=True,
        clean_tmp_tiles=True,
    ):
        ...
        if ba_method is None:
            ...
            corrected_aoi_fn = os.path.join(
                self.dst_dir, "{}/AOI_adj.pickle".format(ba_method)
            )
            corrected_aoi_lonlat = loader.load_pickle(corrected_aoi_fn)
        ...
        src_config_fnames = [
            fn.replace("dsm.tif", "config.json") for fn in s2p_dsm_fnames
        ]
        dsm_res = float(
            loader.load_dict_from_json(src_config_fnames[0])["dsm_resolution"]
        )
        mask = loader.get_binary_mask_from_aoi_lonlat_within_utm_bbx(
        ...
        for d_idx, t_idx in enumerate(timeline_indices):
            ...
            stereo_dsms_fnames = loader.load_s2p_dsm_fnames_from_dir(
                "{}/{}/{}".format(*args)
            )
            if use_cdsms:
                stereo_dsms_fnames = [
                    fn.replace("dsm.tif", "cdsm.tif") for fn in stereo_dsms_fnames
                ]

            if pc3dr:
                out_dir = os.path.join(
                    rec4D_dir, "pc3dr/metrics/{}_per_date".format(stat)
                )
            else:
                out_dir = os.path.join(rec4D_dir, "metrics/{}_per_date".format(stat))

            ba_metrics.compute_stat_for_specific_date_from_tiles(
                complete_dsm_fname,
                stereo_dsms_fnames,
                output_dir=out_dir,
                stat=stat,
            ...
            if stat == "avg" and pc3dr:
                ...
                tmp = complete_dsm_fname.replace(
                    os.path.dirname(complete_dsm_fname), out_dir
                )
                ...
                os.system(
                    "bin/ncc_apply_shift {} `cat {}` {}".format(tmp, trans, dsm_r)
                )

            args = [t_id, stat, d_idx + 1, len(timeline_indices)]
            print(
                "{} - done computing {} multi-view DSM ({}/{})\n".format(*args),
                flush=True,
            )
        ...
        if stat == "avg" and pc3dr:
            ...
            os.system(
                "mv {0}/pc3dr/metrics/avg_per_date_r {0}/pc3dr/metrics/avg_per_date".format(
                    rec4D_dir
                )
            )
    ...
    def project_pts3d_adj_onto_dsms(
        self, timeline_indices, ba_method, geotiff_label=None
    ):
        ...
        for d_idx, t_idx in enumerate(timeline_indices):
            t_id = self.timeline[t_idx]["id"]
            if ba_method == "ba_sequential":
                ply_path = "{}/{}/pts3d_adj/{}_pts3d_adj.ply".format(
                    self.dst_dir, ba_method, t_id
                )
            else:
            ...
            ba_utils.save_ply_pts_projected_over_geotiff_as_svg(
                dsm_path, ply_path, svg_path
            )
            args = [d_idx + 1, len(timeline_indices)]
            print(
                "Projecting adjusted 3d points onto output multi-view DSMs... {}/{}".format(
                    *args
                ),
                flush=True,
            )
    ...
    def interpolate_small_holes(
        ...
        for t_idx in timeline_indices:
            s2p_dir = "{}/{}/{}".format(
                rec4D_dir, "pc3dr/s2p" if pc3dr else "s2p", self.timeline[t_idx]["id"]
            )
            ...
            for dsm_idx, dsm_fn in enumerate(dsm_fnames):
                ...
                print(
                    "\r{} - interpolating stereo DSMs: {}/{}".format(*args),
                    end="\r",
                    flush=True,
                )
            print("\n")
    ...
    def update_aoi_after_bundle_adjustment(self, ba_dir):
        ...
        init_rpc_fn = "{}/RPC_init/{}".format(
            self.dst_dir, os.path.basename(ba_rpc_fn).replace("_RPC_adj", "_RPC")
        )
        init_rpc = rpcm.rpc_from_rpc_file(init_rpc_fn)
        ba_rpc = rpcm.rpc_from_rpc_file(ba_rpc_fn)
        corrected_aoi = ba_utils.reestimate_lonlat_geojson_after_rpc_correction(
            init_rpc, ba_rpc, self.aoi_lonlat
        )
        loader.save_pickle("{}/AOI_adj.pickle".format(ba_dir), corrected_aoi)

    def update_config_json_after_bundle_adjustment(
        self, src_config_fname, dst_config_fname, ba_method, verbose=False
    ):
        ...
        loader.save_dict_to_json(
            config_s2p, dst_config_fname.replace("config.json", "config_src.json")
        )
        ...
        # correct roi_geojson
        if use_corrected_rpcs:
            img_rpc_path = os.path.join(
                adj_rpc_dir,
                loader.get_id(config_s2p["images"][0]["img"]) + "_RPC_adj.txt",
            )
            corrected_rpc = rpcm.rpc_from_rpc_file(img_rpc_path)
            initial_rpc = rpcm.RPCModel(
                config_s2p["images"][0]["rpc"], dict_format="rpcm"
            )
            roi_lonlat_init = config_s2p["roi_geojson"]
            roi_lonlat_ba = ba_utils.reestimate_lonlat_geojson_after_rpc_correction(
                initial_rpc, corrected_rpc, roi_lonlat_init
            )
        ...
        for i in [0, 1]:
            ...
            img_geotiff_path = glob.glob(
                "{}/**/{}".format(self.geotiff_dir, img_basename), recursive=True
            )[0]
        ...
        if verbose:
            for i, c in zip([0, 1], ["r", "b"]):
                img_rpc_path = os.path.join(
                    adj_rpc_dir,
                    loader.get_id(config_s2p["images"][i]["img"]) + "_RPC_adj.txt",
                )
                ...
                roi_cols_ba, roi_rows_ba = correct_rpc.projection(
                    roi_lons_ba, roi_lats_ba, [alt] * roi_lons_ba.shape[0]
                )
                ...
                current_roi = {
                    ...
                    "w": int(np.floor(x_max - x_min) + 1),
                    "h": int(np.floor(y_max - y_min) + 1),
                }
                ...
                rect = patches.Rectangle(
                    (0, 0), w, h, linewidth=5, edgecolor=c, facecolor="none"
                )
                ax.add_patch(rect)
        ...
        if verbose:
            ...
            for i in [0, 1]:
                initial_rpc = rpcm.RPCModel(
                    config_s2p["images"][i]["rpc"], dict_format="rpcm"
                )
                ...
                current_footprint = ba_utils.get_image_footprints(
                    [initial_rpc], [current_offset]
                )[0]
                ...
                correct_rpc = rpcm.rpc_from_rpc_file(img_rpc_path)
                current_footprint = ba_utils.get_image_footprints(
                    [correct_rpc], [current_offset]
                )[0]
                utm_polys_ba.append(current_footprint["poly"])

            aoi_utm_init = shape(
                geotools.utm_geojson_from_lonlat_geojson(self.aoi_lonlat)
            )
            aoi_utm_ba = shape(
                geotools.utm_geojson_from_lonlat_geojson(self.corrected_aoi_lonlat)
            )
            # utm_polys_init.append(aoi_utm_init)
            # utm_polys_ba.append(aoi_utm_ba)

            roi_utm_init = shape(
                geotools.utm_geojson_from_lonlat_geojson(roi_lonlat_init)
            )
            ...
            for (this_ax, utm_polys_to_display) in zip(
                ax, [utm_polys_init, utm_polys_ba]
            ):
                for shapely_poly, color in zip(
                    utm_polys_to_display, ["r", "b", "g", "r"]
                ):
        ...
        for i in [0, 1]:
            ...
            if use_corrected_rpcs:
                ...
                config_s2p["images"][i]["rpc"] = rpcm.rpc_from_rpc_file(
                    img_rpc_path
                ).__dict__
            else:
        ...
        if not "out_crs" in config_s2p.keys() and "utm_zone" in config_s2p.keys():
            config_s2p["out_crs"] = "epsg:{}".format(
                loader.epsg_from_utm_zone(config_s2p["utm_zone"])
            )
    ...
    def run_pc3dr(self, timeline_indices, ba_method=None, std=True):
        ...
        s2p_dirs = [
            "{}/s2p/{}".format(rec4D_dir, self.timeline[t_idx]["id"])
            for t_idx in timeline_indices
        ]
        flatten_list = lambda t: [item for sublist in t for item in sublist]
        s2p_dsm_fnames = flatten_list(
            [loader.load_s2p_dsm_fnames_from_dir(path) for path in s2p_dirs]
        )
        ...
        with open(log_file, "w") as outfile:
            subprocess.run(
                ["pc3dr"] + input_dirs_pc3dr + ["--outdir", tmp_dir],
                stdout=outfile,
                stderr=outfile,
            )
        ...
        for d_idx, (t_idx, path_to_registered_ply_files) in enumerate(
            zip(timeline_indices, s2p_dirs)
        ):
            ply_list = glob.glob(
                path_to_registered_ply_files + "/**/cloud_registered.ply",
                recursive=True,
            )
            output_dsm_path = os.path.join(
                "{}/{}.tif".format(output_dir_pc3dr, self.timeline[t_idx]["id"])
            )
            ba_utils.run_plyflatten(
                ply_list,
                dsm_resolution,
            ...
            print(
                "\rmerging registered DSMs... ({}/{}) {} done".format(*args),
                end="\r",
                flush=True,
            )
        print("\n", flush=True)

    def run_pc3dr_datewise(
        self,
        timeline_indices,
        ba_method=None,
        std=True,
        ...
        if intradate:
            ...
            for k, (t_idx, s2p_dir) in enumerate(zip(timeline_indices, s2p_dirs)):
                ...
                print(
                    "...({}/{}) pc3dr to align {} [{} DSMs]".format(
                        k + 1, n_dates, t_id, n_dsms
                    ),
                    flush=True,
                )
                ...
                with open(log_file, "w") as outfile:
                    subprocess.run(
                        ["pc3dr"] + in_dirs_pc3dr + ["--outdir", tmp_dir],
                        stdout=outfile,
                        stderr=outfile,
                    )
                ...
                for fn in s2p_dsm_fnames:
                    ply_list = glob.glob(
                        os.path.dirname(fn) + "/**/cloud_registered.ply", recursive=True
                    )
                    ...
                    (
                        ...
                        _,
                        _,
                    ) = loader.read_geotiff_metadata(fn)
                    ba_utils.run_plyflatten(
                        ply_list,
                        dsm_res,
                        tmp_utm_bbx,
                ...
                # complete dsm
                ply_list = glob.glob(
                    s2p_dir + "/**/cloud_registered.ply", recursive=True
                )
                if len(ply_list) > 0:
                    ba_utils.run_plyflatten(
                        ply_list,
                ...
                os.system(
                    "rm -r {}".format(tmp_dir)
                )  # clean temp files, otherwise next run of pc3dr will crash
                time_to_print = loader.get_time_in_hours_mins_secs(
                    timeit.default_timer() - start
                )
            ...
            print(
                "\nAll pc3dr runs completed in {} ({} crashes)".format(
                    time_to_print, len(crashes)
                ),
                flush=True,
            )
        else:
        ...
        for iter_cont, dsm in enumerate(dsm_fnames):
            ...
            cmd_2 = (
                'bin/plambda {} - "x isfinite x y isfinite y nan if if" -o {}'.format(
                    dsm, cdsm
                )
            )
            ...
            mcdsm = (
                in_dir + "/mcdsms/mc" + filename
            )  # dsm after closing and min interpolation
        ...
        for iter_cont, dsm in enumerate(dsm_fnames):
            ...
            trans = (
                in_dir + "/ncc_transforms/t_" + os.path.splitext(filename)[0] + ".txt"
            )

            # compute horizontal registration on the interpolated DSMs
            os.system(
                "bin/ncc_compute_shift {} {} 5 > {}".format(ref_mcdsm, mcdsm, trans)
            )
            dx, dy = np.loadtxt(trans)[:2]
            # compute vertical registration on the original DSMs
            os.system(
                "bin/ncc_compute_shift {} {} 5 {} {} > {}".format(
                    ref_cdsm, cdsm, dx, dy, trans
                )
            )
            ...
            # apply mask to dsm
            with rasterio.open(
                os.path.join(rec4D_dir, "dsms/{}".format(filename))
            ) as src_dataset:
    ...
    def compute_reprojection_error_after_bundle_adjust(self, ba_method):
        ...
        rpcs_init = loader.load_rpcs_from_dir(
            im_fnames, rpcs_init_dir, suffix="RPC", verbose=False
        )
        rpcs_ba_dir = os.path.join(self.dst_dir, ba_method + "/RPC_adj")
        rpcs_ba = loader.load_rpcs_from_dir(
            im_fnames, rpcs_ba_dir, suffix="RPC_adj", verbose=False
        )

        # triangulate
        from bundle_adjust.ba_triangulate import init_pts3d

        pts3d_before = init_pts3d(
            C, rpcs_init, cam_model, pairs_to_triangulate, verbose=False
        )
    ...
    def compute_dsm_registration_metrics(
        self,
        timeline_indices,
        ba_method=None,
        over_time=True,
        ...
        std_per_date_dir = "{}{}{}".format(
            rec4D_dir, "/pc3dr/" if pc3dr else "/", dir_suffix
        )
        std_per_date_files = [
            "{}/{}.tif".format(std_per_date_dir, t_id) for t_id in t_ids
        ]
        stacked_std_per_date = np.dstack(
            [np.array(Image.open(fn)) for fn in std_per_date_files]
        )
        avg_std_per_date = [
            np.nanmean(stacked_std_per_date[:, :, i], axis=(0, 1))
            for i in range(n_dates)
        ]

        if over_time:
            std_over_time_dir = "{}{}{}".format(
                rec4D_dir, "/pc3dr/" if pc3dr else "/", "metrics_over_time"
            )
            std_over_time_file = os.path.join(std_over_time_dir, "std_over_time.tif")
            std_over_time = np.array(Image.open(std_over_time_file))
            finite_values_below_1 = np.sum(
                std_over_time.flatten()[~np.isnan(std_over_time.flatten())] < 1.0
            )
            pct = (
                finite_values_below_1
                / np.sum(1 * ~np.isnan(std_over_time.flatten()))
                * 100.0
            )
        print(
        ...
        if over_time:
            ...
            print(
                "mean std over time: {:.3f}".format(
                    np.nanmean(std_over_time, axis=(0, 1))
                ),
                flush=True,
            )
            ...
            print(
                "mean std over time [pts < 1 m]: {:.3f}".format(std_over_time_static),
                flush=True,
            )
        print(
            "mean std per date: {:.3f}\n".format(np.nanmean(avg_std_per_date)),
            flush=True,
        )
