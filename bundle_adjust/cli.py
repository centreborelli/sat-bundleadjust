...
def main():
    ...
    parser.add_argument(
        ...
        metavar="config.json",
        help=(
            "path to a json file containing the necessary "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "just print the timeline of the scene described "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "print all information of the feature tracking, "
    ...
    parser.add_argument(
        "--reset",
        action="store_true",
        help=("delete previous files " "and run all from zero."),
    )
    ...
    os.system(
        "cp {} {}".format(
            args.config, os.path.join(opt["output_dir"], os.path.basename(args.config))
        )
    )

    # redirect all prints to a bundle adjustment logfile inside the output directory
    log_file = open(
        "{}/{}_BA.log".format(opt["output_dir"], data_loader.get_id(args.config)), "w+"
    )
    sys.stdout = log_file
    sys.stderr = log_file
    ...
    opt["geotiff_label"] = (
        opt["geotiff_label"] if "geotiff_label" in opt.keys() else None
    )
    ...
    opt["ref_cam_weight"] = (
        float(opt["ref_cam_weight"]) if "ref_cam_weight" in opt.keys() else 1.0
    )
    opt["filter_outliers"] = (
        float(opt["filter_outliers"]) if "filter_outliers" in opt.keys() else True
    )
    ...
    if opt["reconstruct"]:
        ...
        log_file = open(
            "{}/{}_3D.log".format(opt["output_dir"], data_loader.get_id(args.config)),
            "w+",
        )
        sys.stdout = log_file
        sys.stderr = log_file

        scene.reconstruct_dates(
            timeline_indices,
            ba_method=opt["ba_method"],
            n_s2p=opt["s2p_parallel"],
            geotiff_label=opt["geotiff_label"],
        )

        if opt["ba_method"] is not None:
            scene.project_pts3d_adj_onto_dsms(
                timeline_indices, opt["ba_method"], geotiff_label=opt["geotiff_label"]
            )

        # postprocess dsms and compute registration metrics
        if opt["postprocess"]:
            scene.interpolate_small_holes(
                timeline_indices,
        ...
        scene.compute_stat_per_date(
            timeline_indices,
            ba_method=opt["ba_method"],
            stat="avg",
            use_cdsms=opt["postprocess"],
            geotiff_label=opt["geotiff_label"],
            clean_tmp_warps=False,
        )
        scene.compute_stat_per_date(
            timeline_indices,
            ba_method=opt["ba_method"],
        ...
        scene.compute_stats_over_time(
            timeline_indices,
            ba_method=opt["ba_method"],
            use_cdsms=False,
            geotiff_label=opt["geotiff_label"],
        )
        scene.compute_dsm_registration_metrics(
            timeline_indices,
            ba_method=opt["ba_method"],
            use_cdsms=opt["postprocess"],
            geotiff_label=opt["geotiff_label"],
