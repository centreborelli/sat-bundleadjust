...
from feature_tracks import skysat_utils as fd_skysat

import timeit
import pickle

def run_feature_detection_skysat(data_dir,
                                 n_adj,
                                 n_new,
                                 input_fnames,
                                 input_seq,
        ...
        print('Running skysat {} based feature detection...\n'.format(feature_detection_lib))
        print('Parameters:')
        ...
        new_input_rpcs = [input_rpcs[idx] for idx in new_indices]
        new_footprints = [input_footprints[idx] for idx in new_indices]
        if input_masks is not None and use_masks:
            new_masks = [input_masks[idx] for idx in new_indices]
        else:
            new_masks = None  
        if feature_detection_lib == 's2p':
            new_features = fd_skysat.s2p_feature_detection_skysat(new_input_seq, new_input_rpcs, new_footprints, new_masks)
        else:
            new_features = fd_skysat.opencv_feature_detection_skysat(new_input_seq, new_input_rpcs, new_footprints, new_masks)
        ...
        pairs2match, pairs2triangulate = fd_skysat.filter_pairs_to_match_skysat(possible_pairs, input_footprints, input_P)
        ...
        for pair in pairs2match:
            new_pair= pair.copy()
            new_pair['im_i'], new_pair['im_j'] = indices_img_global[pair['im_i']], indices_img_global[pair['im_j']]
            all_pairs_to_match.append(pair)
        ...
        print('\nMatching...\n')
        if feature_detection_lib == 's2p':
            new_pairwise_matches = fd_skysat.s2p_matching_skysat(pairs2match, features, input_seq, input_rpcs)
        else:
            new_pairwise_matches = fd_skysat.opencv_matching_skysat(pairs2match, features, matching_thr) 
